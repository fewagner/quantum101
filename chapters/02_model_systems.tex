...
Almost every system encountered in circuit QED is built --- exactly or
approximately --- from two ingredients: the quantum harmonic
oscillator and the two-level system (TLS). The harmonic oscillator
describes lumped LC resonators, microwave cavities, transmission-line
modes and, to leading order, weakly anharmonic qubits. The two-level
system describes spins, idealised qubits and any pair of levels we
isolate from a more complicated spectrum. This chapter solves both
exactly, fixes the notation we will use everywhere later, and ends by
combining a TLS with a coherent drive --- producing the Rabi
oscillations that underlie all single-qubit control.
...
\begin{equation}
\label{eq:ladder-def}
a   = \sqrt{\frac{m\omega}{2\hbar}}\Bigl(\op{x}+\frac{i}{m\omega}\op{p}\Bigr),
\qquad
a^\dagger = \sqrt{\frac{m\omega}{2\hbar}}\Bigl(\op{x}-\frac{i}{m\omega}\op{p}\Bigr).
\end{equation}
Using $[\op{x},\op{p}]=i\hbar$ one immediately obtains
\begin{equation}
[a,a^\dagger] = 1,\qquad
\op{H} = \hbar\omega\bigl(a^\dagger a + \tfrac12\bigr).
\end{equation}
The number operator $\op{N}=a^\dagger a$ commutes with $\op{H}$, so
they share an eigenbasis. From $[\op{N},a]=-a$ and $[\op{N},a^\dagger]=+a^\dagger$
one finds that $a$ and $a^\dagger$ shift the eigenvalue of $\op{N}$ by $\mp 1$:
\begin{equation}
...
\end{equation}
Positivity, $\bra{\psi}a^\dagger a\ket{\psi}\geq0$, forces $n\geq0$; a
unique ground state $\ket{0}$ with $a\ket{0}=0$ exists, and the
...
$\sigma_\pm = (\sigma_x\pm i\sigma_y)/2$, with
$\sigma_+\ket{1}=\ket{0}$, $\sigma_-\ket{0}=\ket{1}$ in the convention
$\sigma_z\ket{0}=+\ket{0}$ used throughout these notes.
...
\begin{equation}
\label{eq:tls-h}
\op{H} \;=\; \tfrac{1}{2}\,\bm{B}\cdot\bm{\sigma},
\end{equation}
for some real vector $\bm{B}=(B_x,B_y,B_z)\in\R^3$.
\end{statement}

The eigenvalues of \cref{eq:tls-h} are $\pm |\bm{B}|/2$, and the
eigenvectors are aligned with $\pm\bm{B}/|\bm{B}|$ in the geometric
sense made precise next.

\paragraph{Transition.}
The vector $\bm{B}$ in \cref{eq:tls-h} suggests a geometric picture.
That picture is the Bloch sphere, and it turns the algebra of a TLS
into elementary geometry on $S^2$.
...
\begin{equation}
\label{eq:bloch-rho}
\op{\rho} \;=\; \tfrac12\bigl(\id + \bm{r}\cdot\bm{\sigma}\bigr),
\qquad
...
$\{\id,\sigma_x,\sigma_y,\sigma_z\}$. Trace-one and Hermiticity fix
the coefficient of $\id$ and the reality of the others; positivity
$\op{\rho}\geq0$ requires both eigenvalues
...
$\bm{B}$ at angular speed $|\bm{B}|/\hbar$. Density matrices and the
appearance of $|\bm{r}|<1$ for mixed states are revisited in
\cref{ch:composite}.
...
\begin{equation}
R_{\hat{\bm{n}}}(\phi) = \exp\!\bigl(-i\tfrac{\phi}{2}\,\hat{\bm{n}}\cdot\bm{\sigma}\bigr)
= \cos(\phi/2)\,\id - i\sin(\phi/2)\,\hat{\bm{n}}\cdot\bm{\sigma}.
\end{equation}
...
\begin{equation}
\label{eq:driven-tls}
\op{H}(t) = \frac{\omega_q}{2}\sigma_z + \Omega\cos(\omega_d t)\,\sigma_x,
\end{equation}
with qubit frequency $\omega_q$, drive frequency $\omega_d$ and
drive amplitude $\Omega$.

\paragraph{Move to the rotating frame.}
Apply the unitary $\op{U}_0(t)=e^{-i\omega_d t\sigma_z/2}$. Using
$\op{U}_0^\dagger\sigma_+\op{U}_0=e^{+i\omega_d t}\sigma_+$ and the
identity $\cos(\omega_d t)\sigma_x = \tfrac12 e^{i\omega_d t}\sigma_-+
\tfrac12 e^{-i\omega_d t}\sigma_+ + \tfrac12 e^{-i\omega_d t}\sigma_-+
\tfrac12 e^{i\omega_d t}\sigma_+$, the rotating-frame Hamiltonian
becomes (after dropping fast counter-rotating terms; this is the
\emph{rotating-wave approximation} of \cref{sec:rwa})
\begin{equation}
\label{eq:rabi-h-rwa}
\op{H}_R = \frac{\Delta}{2}\sigma_z + \frac{\Omega}{2}\sigma_x,
\qquad
\Delta = \omega_q - \omega_d.
\end{equation}
This is exactly the form \cref{eq:tls-h} with
$\bm{B}=(\Omega,0,\Delta)$, so the Bloch vector precesses about
$\bm{B}$ at the \emph{generalised Rabi frequency}
\begin{equation}
...
$\exp(-i\op{H}_R t)=\cos(\Omega_R t/2)\id - i\sin(\Omega_R t/2)\,
\hat{\bm{n}}\cdot\bm{\sigma}$ with
$\hat{\bm{n}}=(\Omega,0,\Delta)/\Omega_R$. Acting on $\ket{0}$ and
projecting onto $\ket{1}$ gives \cref{eq:rabi-formula}.
\end{proof}
