...
$\to$ \emph{statistical limit on the model parameter} --- and
ends with a short tour of how the four cQED-based sensing
mechanisms covered earlier in the book sit on each channel. Most of
the chapter is detector-agnostic; specialisations to superconducting
hardware appear as worked numerical examples and in the closing
\cref{sec:cqed-mechanisms}.
...
and enhancing the small-$q$ region. For DM--electron scattering an
analogous formula holds with the bound-electron wavefunction
replacing the form factor; the atomic-physics details are reviewed
in~\cite{essig2012}.
\end{remark}
...
\begin{phenomenon}
The detector sees not a single recoil but a distribution of recoils
sampled over the velocity distribution of the DM halo. The shape
of $\dd R/\dd E_R$ is the central observable of every recoil
search.
\end{phenomenon}

\begin{statement}[Velocity-averaged differential rate]
\label{stmt:dRdE}
For a target of total mass $\mathcal M$ comprised of nuclei of
individual mass $M_T$ and number density $n_T=\mathcal M/(m_T \cdot V)$,
...
read off straightforwardly from \cref{eq:flux},
\cref{eq:halo-distribution} and \cref{eq:dsigma-dE-SI}; the
$v_\text{min}$ lower limit on the velocity integral expresses the
kinematic requirement \cref{eq:v-min}.
\end{proof}

\paragraph{Mean-inverse-speed function.}
For the standard halo model, the velocity integral in
\cref{eq:dRdE_R} is conventionally written through the
mean-inverse-speed function
\begin{equation}
\label{eq:eta}
...
\cref{eq:dsigma-dE-SI} into \cref{eq:dRdE_R}, the differential rate
takes the often-quoted form
\begin{equation}
\label{eq:dRdE-final}
\frac{1}{\mathcal M}\,\frac{\dd R}{\dd E_R}
...
\begin{example}[Recoil spectrum on silicon]
...
\paragraph{Transition.}
We have walked both channels through the four-stage pipeline. The
final section collects the four cQED-based sensing mechanisms
discussed earlier in this book and maps each onto the
recoil--vs--absorption dichotomy.

% --------------------------------------------------------------
\section{Sensing mechanisms in superconducting circuits}
\label{sec:cqed-mechanisms}

The cQED toolkit provides four practical implementations of the
recoil and absorption channels:

\begin{itemize}
  \item \emph{Phonon-quasiparticle conversion} (recoil channel).
  A target deposits athermal phonons that, above the
  $2\Delta\sim 0.34\,\text{meV}$ Cooper-pair binding energy in
  aluminium, break Cooper pairs and create quasiparticles read out
  by a transition-edge or kinetic-inductance
  sensor~\cite{kurinsky2020}. Threshold $E_\text{th}\sim$
  meV, mass reach $m_\chi\sim 10\,\text{MeV}$ for nuclear recoils.
  \item \emph{Infrared absorption in the junction}
  (absorption channel). A Josephson junction directly absorbs an
  infrared or terahertz photon of energy $\geq 2\Delta$, with the
  resulting quasiparticle pair read out as a poisoning
  burst~\cite{hochberg2021}. Sensitive to dark photons with
  $m_{A'}\geq 0.34\,\text{meV}/c^2$ at couplings down to
  $\varepsilon\sim 10^{-13}$.
  \item \emph{Phonon-qubit coupling via piezoelectric substrates}
  (recoil channel). A piezoelectric substrate (AlN, GaAs)
  converts a single phonon into a charge perturbation that drives
  a transmon transition; the transmon acts as a single-phonon
  detector with $g/2\pi\sim$ MHz~\cite{linehan2024}. Mass reach
  $m_\chi\sim 1\,\text{MeV}$ for nuclear recoils.
  \item \emph{Microwave absorption in the qubit} (absorption
  channel). A transmon tuned to $\omega_q=m_\chi c^2/\hbar$ acts as
  a single-microwave-photon counter for dark-photon or axion
  haloscopes~\cite{dixit2021}. Sensitivity $\varepsilon\sim 10^{-15}$
  in the GHz band.
\end{itemize}

The first and third belong to the recoil pipeline of
\cref{sec:fermion-dm}; the second and fourth belong to the
absorption pipeline of \cref{sec:boson-dm}. The detector-response
and statistical machinery of \cref{sec:detector-response} and
\cref{sec:limits} apply uniformly to all four.

% --------------------------------------------------------------
The chapter walked through two distinct dark-matter detection
...
dispersion to $\Delta\omega/\omega\sim 10^{-6}$. Either signal, after
folding with the detector response, becomes a Poisson count whose
expectation depends on the coupling parameter; thresholding a
likelihood produces an exclusion curve in the coupling-mass plane.
The four cQED-based sensing mechanisms covered in earlier chapters
populate both pipelines, and the methodology of this chapter
...
\paragraph{cQED sensing mechanisms (\cref{sec:cqed-mechanisms}).}
Phonon-quasiparticle conversion and piezoelectric phonon-qubit
coupling sit on the recoil pipeline; IR absorption in the junction
and microwave absorption in the transmon sit on the absorption
pipeline.
