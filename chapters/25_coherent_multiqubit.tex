...
(\cref{ch:qec}) is exactly that QEC
...
\begin{note}
The ``standard quantum limit'' meant here is the \emph{metrological}
one --- the $1/\sqrt N$ floor on phase estimation with independent
probes. It is a distinct statement from the \emph{amplifier} quantum
limit of \cref{stmt:parametric-amp} (the half-quantum of added noise,
$n_{\rm add}\ge\tfrac12$) and from the vacuum-noise limit of the
quadratures (\cref{stmt:quadratures}): the three share a name but bound
...
the chapter on quantum error correction (\cref{ch:qec}) states the
threshold theorem: arbitrary fault tolerance is achievable for
