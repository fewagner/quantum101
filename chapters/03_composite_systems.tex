...
$\hilbert_{AB}=\hilbert_A\otimes\hilbert_B$. If $\{\ket{i}_A\}$ and
$\{\ket{j}_B\}$ are bases of $\hilbert_A,\hilbert_B$ then
$\{\ket{i}_A\otimes\ket{j}_B\}$ is a basis of $\hilbert_{AB}$.
Operators acting only on $A$ or $B$ are extended by the identity:
$\op{O}_A\to\op{O}_A\otimes\id_B$, $\op{O}_B\to\id_A\otimes\op{O}_B$.
\end{postulate}
