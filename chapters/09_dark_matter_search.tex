...
\begin{remark}[Numerical evaluation: \texttt{DarkELF}]
...
hydrogen-like atom). For real semiconductor and metal targets
it is computed numerically from density-functional theory or from
extrapolations of optical-conductivity data. The
\texttt{DarkELF} package~\cite{knapen2021} packages tabulated
$\mathcal{W}(q,\omega)$ for Si, Ge, GaAs, GaN, SiC, sapphire and
aluminium, together with python routines that wrap
\cref{eq:dRdE-electron}, \cref{eq:vmin-electron} and the
mean-inverse-speed function \cref{eq:eta} into a single
\texttt{rate.dRdomega(omega, mX, sigma\_e, F\_DM)} call. The
practical workflow for a DM--electron analysis is therefore: pick
a target, pick a DM model (mediator + reference $\bar\sigma_e$),
call \texttt{DarkELF} on a grid in $\omega$, and feed the
resulting $\dd R/\dd\omega$ into the detector-response formula
\cref{eq:N-exp} of \cref{sec:detector-response}. The numerical
accuracy of $\mathcal{W}$ is limited by the underlying
band-structure calculation; in regions where independent
optical-loss data exist, agreement is at the
$\mathcal O(10\,\%)$ level.
\end{remark}
...
For $m_\chi=10\,\text{MeV}$, $\bar\sigma_e=10^{-37}\,\text{cm}^2$, a
heavy mediator ($F_\text{DM}=1$), and a silicon target
($\rho_T=2.33\,\text{g/cm}^3$), \cref{eq:dRdE-electron} evaluated
with the \texttt{DarkELF}~\cite{knapen2021} ELF for crystalline
silicon yields a spectrum dominated by the bulk-plasmon resonance
near $\omega\approx 17\,\text{eV}$, with a low-energy tail above
the indirect band gap of $E_g\approx 1.1\,\text{eV}$. The
characteristic kinematic threshold is set by demanding that
$v_\text{min}(q,\omega)\leq v_\text{esc}$ in
\cref{eq:vmin-electron}; with
$\omega/q\sim 10^{-3}\,c$ around the gap, this places the lowest
detectable DM mass at $m_\chi^\text{min}\sim E_g/(2 v_\text{esc}^2 m_e)\,m_e
\approx 0.5\,\text{MeV}$ for $\omega=E_g$. A detector with
