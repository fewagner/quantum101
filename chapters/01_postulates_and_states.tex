...
product, $\braket{U\phi}{U\psi}=\braket{\phi}{\psi}$, and therefore
preserve probabilities; and as the operators that implement reversible
time evolution (\cref{sec:time-evolution}).
\end{remark}
...
\begin{example}[Measuring $\sigma_z$ on a qubit]
Take $\op{A}=\sigma_z$ with eigendecomposition
$\sigma_z=(+1)\ket{0}\bra{0}+(-1)\ket{1}\bra{1}$. For
$\ket{\psi}=\cos(\theta/2)\ket{0}+e^{i\varphi}\sin(\theta/2)\ket{1}$
...
For $\op{H}_0=\tfrac{\omega_q}{2}\sigma_z$ and a drive
$\op{V}(t)=\Omega\cos(\omega_d t)\,\sigma_x$, moving to the
interaction picture with respect to $\op{H}_0$ rotates
$\sigma_\pm$ at the qubit frequency $\omega_q$, isolating the
