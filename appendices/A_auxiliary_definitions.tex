...
representation theorem identifies $V^*$ canonically with $V$ itself:
for every $\phi\in V^*$ there is a unique $\chi_\phi\in V$ with
$\phi(\psi)=\langle\chi_\phi,\psi\rangle$. Dirac notation makes
this identification explicit: $\bra{\phi}\in V^*$ is the linear
functional that sends $\ket{\psi}$ to $\braket{\phi}{\psi}$.
\end{definition}
