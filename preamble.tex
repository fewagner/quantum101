...
% Cleveref otherwise reports every theorem-like environment as
% "theorem N.M" because they share the theorem counter; declare a
% per-environment name so cref reports "Definition / Example /
% Statement / ..." correctly.
...
% ---- Chapter summary box ----
% Used at the end of each chapter to highlight the take-home
% concepts and formulas in a single full-frame coloured box.
\mdfdefinestyle{summarybox}{%
  topline=true, bottomline=true, leftline=true, rightline=true,
  linecolor=primary, linewidth=1pt,
  backgroundcolor=primary!4,
  innertopmargin=14pt, innerbottommargin=14pt,
  innerleftmargin=14pt, innerrightmargin=14pt,
  skipabove=18pt, skipbelow=12pt,
}
\newenvironment{chaptersummary}{%
  \begin{mdframed}[style=summarybox]%
  \begin{center}
    {\sffamily\bfseries\Large\color{primary}Chapter takeaways}
  \end{center}
  \vspace{0.3em}\par
}{%
  \end{mdframed}%
...
% ---- Exercises ----
% A lightweight "Exercises" environment for the end of each chapter.
% Each exercise gets a numbered heading followed by the problem,
% then a "Solution" block. Solutions are shown by default; switch
% to a hide-solutions mode by changing the \ifshowsolutions flag.
...
\newenvironment{exercise}[1][]{%
  ...
  \par\medskip
  \noindent\textbf{\color{primary}Exercise \theexercise.}%
  \ifx\\#1\\\else\ \emph{\color{primary!80}#1.}\fi
  \par\noindent\ignorespaces
}{\par\medskip}
\newenvironment{solution}{%
  \par\smallskip
  \begingroup
  \color{black!80}\small
  \noindent\textit{\color{accent}Solution.\ }\ignorespaces
}{%
...
% Makefile / CI inject `version.tex` via `git describe --tags --always --dirty`.
% Falls back to "dev" for ad-hoc builds where version.tex is absent.
